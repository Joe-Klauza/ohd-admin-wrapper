...
class ServerMonitor
  ...
  def process_rcon_players(rcon_players, prev_rcon_players)
    ...
    players_gone = prev_rcon_players.reject { |prev| rcon_players.map { |current| current['unique_id'] }.include?(prev['unique_id']) }
    players_joined = rcon_players.reject { |current| prev_rcon_players.map { |prev| prev['unique_id'] }.include?(current['unique_id']) }
    ...
    if @daemon_handle && (@steam_api_client || !@daemon_handle.steam_api_key.empty?)
      ...
      ids_to_query = rcon_players.select { |p| now - $config_handler.players.dig(p['unique_id'], 'last_steam_lookup').to_i > 60 * 60 * 24 }.map { |p| p['unique_id'] }
      ...
      steam_users.each do |user|
        ...
        matching_player = rcon_players.select { |player| player['unique_id'] == user['steamid'] }.first
        matching_player['steam_info'] = {
          ...
          'game_server_unique_id' => user['gameserversteamid'] || '',
    ...
    players_joined.each do |player|
      if @daemon_handle
        is_admin = @daemon_handle.is_ohd_admin?(player['unique_id'])
        ...
        unless @daemon_handle.config[message_option].empty?
          Thread.new(message_option) do |message_option|
            ...
            message = message.gsub('${player_name}', player.dig('steam_info', 'name') || player['name']).gsub('${player_id}', player['unique_id'] || 'NULL')
        ...
        log "Player joined: #{player['name']} (#{player['unique_id']})#{' (admin)' if is_admin}", level: :info
      else
        log "Player joined: #{player['name']} (#{player['unique_id']})", level: :info
      end
    end

    # players_needing_ip_query = rcon_players.select do |p|
    #   now - $config_handler.players.dig(p['unique_id'], 'last_ip_lookup', p['ip']).to_i > 60 * 60 * 24
    # end
    ...
    #   end
    #   begin
    #     log "Looking up IP #{ip} for player #{player['name']} (#{player['unique_id']})"
    ...
    #       log "Couldn't find IP info for #{ip} for player #{player['name']} (#{player['unique_id']})"
    #       next
    #     end
    #     ip_info = ip_info.data
    #     log "Got info about IP #{ip} for player #{player['name']} (#{player['unique_id']}): #{ip_info}"
    #     $config_handler.players[player['unique_id']]['last_ip_lookup'] = {} if $config_handler.players[player['unique_id']]['last_ip_lookup'].nil?
    #     $config_handler.players[player['unique_id']]['ip_info'] = {} if $config_handler.players[player['unique_id']]['ip_info'].nil?
    #     $config_handler.players[player['unique_id']]['last_ip_lookup'][player['ip']] = now
    #     $config_handler.players[player['unique_id']]['ip_info'][player['ip']] = ip_info
    #   rescue => e
    #     log "Failed to look up IP #{player['ip']} (#{player['unique_id']})"
    #   end
    # end

    # Merge the current data
    rcon_players.map do |player|
      if $config_handler.players[player['unique_id']].nil?
        $config_handler.players[player['unique_id']] = player
      else
        $config_handler.players[player['unique_id']].merge!(player) # Merge the current data
        player.merge!($config_handler.players[player['unique_id']]) # Merge back the Steam info in case we don't have it
      end
      player
    end
    ...
    players_gone.each do |player|
      if @daemon_handle
        is_admin = @daemon_handle.is_ohd_admin?(player['unique_id'])
        log "Player left: #{player['name']} (#{player['unique_id']})#{' (admin)' if is_admin}", level: :info
        unless @daemon_handle.config['leave_message'].empty?
          Thread.new do
            message = @daemon_handle.config['leave_message'].gsub('${player_name}', player.dig('steam_info', 'name') || player['name']).gsub('${player_id}', player['unique_id'] || 'NULL')
            @rcon_client.send(@ip, @rcon_port, @rcon_pass, "say #{message}")
          end
        end
      end
      saved_player = $config_handler.players[player['unique_id']]
  ...
  def do_rcon_query
    ...
    if e.message.include?('Invalid gamestate')
      log "Skipping RCON status parsing (map is changing)"
      return
    end
