...
class AdminWrapperSite < Sinatra::Base
  def self.set_up
  ...
  get '/config', auth: :admin do
    ...
    @admins_cfg = File.read ERB.new(CONFIG_FILES[:admins_cfg][:local_erb]).result(binding)
    @mapcycle_cfg = File.read ERB.new(CONFIG_FILES[:mapcycle_cfg][:local_erb]).result(binding)
    @bans_cfg = File.read ERB.new(CONFIG_FILES[:bans_cfg][:local_erb]).result(binding)
  ...
  post '/moderator/:action/:unique_id', auth: :moderator do
    data = Oj.load(request.body.read)
    request.body.rewind
    ...
    when 'ban'
      uuid, buffer = self.class.create_buffer
      Thread.new { @@rcon_client.send(ip, port, password, "banid #{unique_id} #{reason}", buffer: buffer) }
      uuid
    when 'kick'
      uuid, buffer = self.class.create_buffer
      Thread.new { @@rcon_client.send(ip, port, password, "kickid #{unique_id} #{reason}", buffer: buffer) }
      uuid
    else
      status 400
      'Unknown action'
  ...
  get '/download-server-config', auth: :admin do
    ...
    zip_path = File.expand_path File.join CONFIG_FILES_DIR, "ohdaw-config_#{config_id}_#{DateTime.now.strftime('%Y-%m-%d_%H-%M-%S')}.zip"
  ...
  get '/download-server-logs(/:config_id)?', auth: :admin do
    config_id = params['config_id']
    zip_path = File.expand_path File.join SERVER_LOG_DIR, "#{config_id + '-' if config_id}ohd-logs-#{DateTime.now.strftime('%Y-%m-%d_%H-%M-%S')}.zip"
  ...
  get '/download-wrapper-logs', auth: :host do
    zip_path = File.expand_path File.join WEBAPP_ROOT, 'log', "ohdaw-logs-#{DateTime.now.strftime('%Y-%m-%d_%H-%M-%S')}.zip"
...
def get_webrick_options(config = AdminWrapperSite.load_webapp_config)
  webrick_options = {
    ...
    app:       AdminWrapperSite, # app must be lowercase
  ...
  if config['admin_interface_use_ssl']
    ...
    if File.exist?(config['admin_interface_ssl_cert']) && File.exist?(config['admin_interface_ssl_key'])
      ...
      AdminWrapperSite.save_webapp_config(config)
...
begin
  config = AdminWrapperSite.load_webapp_config
