...
class ServerDaemon
  ...
  def is_ohd_admin?(unique_id)
    @admin_ids.include?(unique_id.to_s)
  end
  ...
  def run_game_server
    ...
    @admin_ids = $config_handler.get_server_config_file_content(:admins_cfg, @frozen_config['id']).split("\n").map { |l| l[/\d{17}/] }.compact
    executable = BINARY
            ...
            log.tail do |line|
              ...
              if line.include? 'LogRCON'
                ...
                if line[/LogRcon: Error: Failed to create TcpListener at .* for rcon support/]
                  ...
                  kill_server_process
                elsif !@rcon_listening && line.include?('LogRCON: RCON server listening') && @monitor.nil?
                  @rcon_listening = true
