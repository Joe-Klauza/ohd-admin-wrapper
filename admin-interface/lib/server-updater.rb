...
class ServerUpdater
  ...
  def initialize(server_root_dir, steamcmd_path, steam_appinfovdf_path)
    ...
    @app_manifest = File.join server_root_dir, 'steamapps', 'appmanifest_950900.acf'
  ...
  def get_latest_build_id
    ...
    stdout, stderr, status = Open3.capture3(@steamcmd_path,
      '+login anonymous',
      '+app_info_print 950900',
      '+exit'
    )
    # Convert SteamCMD output to JSON for better traversal
    output = stdout[stdout.index('"950900"')..stdout.index(/^\}$/)]
    ...
    build_id = json.dig('950900', 'depots', 'branches', 'public', 'buildid')
    if build_id.nil?
  ...
  def update_server(buffer=nil, validate: nil, ignore_status: true, ignore_message: true)
    ...
    command = [
      ...
      "+app_update 950900#{' validate' if validate}",
      '+exit'
    ]
