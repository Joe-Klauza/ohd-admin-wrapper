...
class RconClient
  ...
  def send_receive(socket, packet, timeout=3, retries=1, sensitive: false)
    ...
    if header.to_s.empty?
      ...
      # Unlike Insurgency: Sandstorm, some commands have a nil/empty header
      # such as AddBluforBots/AddOpforBots.
      return nil
    end
  ...
  def get_players_and_bots(server_ip, port, password, buffer: nil, ignore_status: false, ignore_message: false, timeout: 2, retries: 1)
    resp = send(server_ip, port, password, 'status', buffer: buffer, ignore_status: ignore_status, ignore_message: ignore_message, timeout: timeout, retries: retries)
    ...
    _, info, players_and_bots, _ =  resp.split("\n========================================\n")
    info = info.split("\n")
    return [], [] if players_and_bots.nil?
    players_and_bots = players_and_bots.split("\n")
    players_and_bots.shift # Remove header row
    players = []
    bots = []
    players_and_bots.each do |pb|
      id, name_and_unique_id = pb.split("\t")
      parts = name_and_unique_id.split(/\s+/)
      unique_id = parts.pop
      name = parts.join
      is_invalid_entry = id.to_s.empty? || unique_id.to_s.empty? || name.to_s.empty?
      throw "Invalid player or bot: #{pb}" if is_invalid_entry
      pb = {
        'id' => id,
        'name' => name,
        'unique_id' => unique_id
...
if __FILE__ == $0
  ...
  players, bots = client.get_players_and_bots(server_ip, port, password)
  binding.pry
end
